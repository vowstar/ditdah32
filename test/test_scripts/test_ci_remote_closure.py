...
def test_ci_remote_closure_stops_when_dispatch_fails(tmp_path, monkeypatch):
    ...
    def fake_run(cmd, cwd, stdout, stderr, check):
        assert cwd == repo_root
        commands.append(cmd)
        assert Path(cmd[1]).name == "ci_remote_dispatch.py"
    ...
    assert [step["name"] for step in report["steps"]] == ["ci_remote_dispatch"]
    assert report["steps"][0]["status"] == "fail"
    assert [Path(cmd[1]).name for cmd in commands] == ["ci_remote_dispatch.py"]
